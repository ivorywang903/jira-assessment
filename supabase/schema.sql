...
--    service = 'app_user'   → 系統登入帳號（username/token 為帳密）
--    service = 'jira'       → Jira 連線設定（username = 固定 'default'）
--    service = 'bitbucket'  → Bitbucket 連線設定（username = 固定 'default'）
CREATE TABLE IF NOT EXISTS auth_info (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  service      TEXT NOT NULL,
  base_url     TEXT NOT NULL DEFAULT '',
  username     TEXT NOT NULL,
  token        TEXT NOT NULL,         -- app_user: SHA-256 hashed password; others: API token
  extra        JSONB,
...
-- RLS（Row Level Security）— 視需求開啟
-- ALTER TABLE auth_info ENABLE ROW LEVEL SECURITY;
